...
def create_rnn_model(sequence_length: int = 30, units: int = 64) -> tf.keras.Model:
    ...
    """
    model = Sequential([
        SimpleRNN(units, activation='tanh', return_sequences=True, 
...
def plot_results(y_true: np.ndarray, y_pred: np.ndarray, title: str = "RNN Predictions") -> None:
    """
...
def main():
    ...
    print(f"\n6. Creating RNN model ({RNN_UNITS} units)...")
    ...
    # Plot results
    print("\n10. Generating plots...")
    plot_results(y_test_original, y_pred, title="Baseline RNN")
    ...
    print(f"Plots saved: figures/baseline_rnn_pred_vs_actual.png, figures/baseline_rnn_error_hist.png")
